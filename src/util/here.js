...
export const here = {
  ...
  search(term, location) {
    ...
    return fetch(geocodeURL)
      ...
      .then(coordinates => {
        const url = `https://places.cit.api.here.com/places/v1/discover/search?app_id=${appId}&app_code=${appCode}&at=${coordinates.lat +
          "%2C" +
          coordinates.lon}&q=${encodedTerm}&pretty`;
        const method = "GET";
      ...
      .then(responseJson => {
        if (responseJson.results.items) {
          ...
          return responseJson.results.items.map(poi => ({
            ...
            ...(poi.hasOwnProperty("openingHours") && {
  ...
  getImage(term) {
    let encodedTerm = encodeURI(term);
    return fetch(
      `https://pixabay.com/api/?key=${pixabayKey}&q=${encodedTerm}&image_type=photo`
    )
      .then(response => {
        return response.json();
      })
  ...
  getMapImage(location) {
    ...
    return fetch(geocodeURL)
      ...
      .then(coordinates => {
        ...
        return fetch(
          `https://image.maps.api.here.com/mia/1.6/mapview?c=${geo}&z=14&w=300&h=300&f=1&app_id=${appId}&app_code=${appCode}`
        );
      });
  }
};
